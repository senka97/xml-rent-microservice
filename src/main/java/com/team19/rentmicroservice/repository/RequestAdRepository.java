...
public interface RequestAdRepository extends JpaRepository<RequestAd, Long> {
    ...
    @Query(value="FROM RequestAd ra inner join ra.request r WHERE r.status='Paid' AND ra.adID=?1 AND ((ra.startDate<?2 AND ra.endDate>=?2) OR (ra.startDate>=?2 AND ra.endDate<=?3) OR (ra.startDate<=?3 AND ra.endDate>?3) OR (ra.startDate<?2 AND ra.endDate>?3))")
